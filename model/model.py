...
def load_dataset():
    ...
    for folder_name, label in zip([cat_folder_name, dog_folder_name], [cat_class_index, dog_class_index]):
        ...
        for index, filename in enumerate(files_list):
            if filename.endswith(images_extension):
                if index % 1000 == 1:
                    print("Loading " + folder_name + " image " + str(index) + " of " + str(files_count))
...
def create_test_set(x, y, test_size=0.1):
    (x_train, y_train), (x_test, y_test), num_classes = split_dataset(x, y, ratio=test_size)
...
def create_validation_set(x_train, y_train, val_size=0.1):
    (x_train, y_train), (x_val, y_val), num_classes = split_dataset(x_train, y_train, ratio=val_size)
...
def train_and_save_model():
    ...
    (x_train, y_train), (x_test, y_test) = create_test_set(x_train, y_train, test_size=0.1)
